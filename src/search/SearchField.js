...
const handleSearchIconClick = () => {
  // Add logic to open the search drawer here
  // For example, you can use a state to manage the drawer's open/closed state
  // and toggle it when the search icon is clicked.
  // setDrawerOpen(true);
  ...
  (
    ...
    return (
      <Root className={classes.root} data-empty={value.trim().length === 0 ? 'on' : 'off'}>
        ...
        <IconButton onClick={handleSearchIconClick} size="large">
          <SearchIcon />
        </IconButton>
          <input
            {...others}
